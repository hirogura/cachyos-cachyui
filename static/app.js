...
async function openSelfcode() {
  try {
    ...
    if (data.installed && data.url) {
      ...
      setTimeout(() => {
        if (ws && ws.readyState === WebSocket.OPEN) {
          const installCmd = 'sudo pacman -S --needed --noconfirm git curl nodejs npm && curl -fsSL https://raw.githubusercontent.com/hirogura/cachyos-selfcode/main/install-selfcode.sh -o /tmp/install-selfcode.sh && sudo bash /tmp/install-selfcode.sh\n';
          ws.send(JSON.stringify({ type: 'input', data: installCmd }));
        } else {
          showStatus('ターミナルに接続できません', 'error');
        }
